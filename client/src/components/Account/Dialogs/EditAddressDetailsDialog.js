...
const EditAddressDetailsDialog = ({ open, onClose, details }) => {
    ...
    const { mutateAsync: putUserAddress, isPending } = useMutation({
        ...
        onSuccess: () => {
            ...
            enqueueSnackbar('Dane adresowe zostały zaktualizowane', { variant: 'success' });
            onClose();
        },
        onError: () => {
            enqueueSnackbar('Wystąpił błąd podczas aktualizacji danych adresowych', { variant: 'error' });
            onClose();
        },
    });
        ...
        <Dialog open={open} onClose={onClose}>
            ...
            <DialogContent>
                ...
                <Controller
                    name='localNo'
                    control={control}
                    render={({ field }) => <TextField {...field} label='Numer lokalu' fullWidth margin='normal' />}
                />
                <Controller
                    name='postCode'
                    control={control}
