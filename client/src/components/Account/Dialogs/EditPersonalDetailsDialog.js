...
/**
 * Komponent dialogowy do edycji danych osobowych.
 *
 * @component
...
const EditPersonalDetailsDialog = ({ open, onClose, details }) => {
  ...
  const { mutateAsync: putUser, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      enqueueSnackbar("Dane osobowe zostały zaktualizowane", {
        variant: "success",
      });
      onClose();
    },
    onError: () => {
      enqueueSnackbar("Wystąpił błąd podczas aktualizacji danych osobowych", {
        variant: "error",
      });
      onClose();
    },
